...
export const wilson = (success: number, total: number): [number, number] => {
  if (!total) return [0, 0];
  const z = 1.96,
    p = success / total,
    d = 1 + (z * z) / total,
...
export const totalTokens = (c: Cell) =>
...
export const pct = (current: number, baseline: number) =>
  baseline === 0
    ? current === 0
      ? 0
      : Infinity
