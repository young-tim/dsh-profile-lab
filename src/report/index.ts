...
export const report = async (dir: string, e: Experiment, cells: Cell[]) => {
  ...
  const per_case = e.variants.flatMap((v) =>
    caseNames.map((c) => {
      const matching = cells.filter(
        (cell) => cell.variant === v.id && cell.case === c,
      );
  ...
  const per_case_comparisons = caseNames.flatMap((caseName) => {
    const baseCase = per_case.find(
      (row) => row.variant === baseline && row.case === caseName,
    )!;
    return per_case
  ...
  const front = e.pricing
    ? pareto(
        variants.filter(
          (x): x is typeof x & { cost: number } => typeof x.cost === "number",
        ),
        (x) => x.pass_rate,
        (x) => x.cost,
  ...
  const failed = cells
    ...
    .map(
      (cell) =>
        `- ${cell.variant}/${cell.case}/${cell.repetition}: ${cell.status}${cell.assertion_failures?.length ? ` (${cell.assertion_failures.join(", ")})` : ""}`,
    );
  const comparisonLines = (rows: typeof comparisons) =>
    rows.length
      ? rows
          .map(
            (v) =>
              `- ${v.variant}: pass ${v.pass_rate_delta_pp.toFixed(2)} pp; median tokens ${typeof v.median_token_delta_pct === "number" ? `${v.median_token_delta_pct.toFixed(2)}%` : "unavailable"}`,
          )
          .join("\n")
      : "None";
